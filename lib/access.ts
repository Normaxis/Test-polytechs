...
export async function currentUser(req:Request):Promise<Account|null>{const token=cookieValue(req);if(!/^[a-f0-9]{64}$/.test(token))return null;const row=await database().prepare('SELECT u.* FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>?').bind(await sha(token),Date.now()).first<DbUser>();return row?account(row):null}
